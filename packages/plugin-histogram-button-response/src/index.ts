 ...
 * The chart is displayed until a response is given. The button itself can be customized using HTML formatting.
...
class HistogramButtonResponsePlugin implements JsPsychPlugin<Info> {
  ...
  private createDescription(html: string, display_element: HTMLElement): void {
    ...
    el.id = "jspsych-histogram-button-response-description";
  ...
  private createCanvas(display_element: HTMLElement): HTMLCanvasElement {
    ...
    container.id = "jspsych-histogram-button-response-chart-container";
  ...
  private computeHistogram(
    ...
    const histogramData = yPercent ? binCounts.map((count) => count / data.length) : binCounts;
    ...
    const highlightBinIndex =
      highlightScore == null
        ? null
        : Math.max(0, Math.min(nBins - 1, Math.floor((highlightScore - minValue) / binWidth)));
  ...
  private renderChart(
    ...
    if (!ctx) {
      throw new Error(
        "plugin-histogram-button-response: Your browser does not support canvas rendering."
      );
    }
  ...
  trial(display_element: HTMLElement, trial: TrialType<Info>): void {
    ...
    // prepare chart data
    const { binLabels, histogramData, backgroundColors, highlightBinIndex } = this.computeHistogram(
      trial.chart_data,
      trial.n_bins,
      trial.y_percent,
