 ...
 * The `densitychart-button-response` plugin displays a density plot chart and records responses generated by a button click.
 * The button itself can be customized using HTML formatting. The density plot stimulus is generated from a numeric array of
 * data, with optional parameters for customizing how smooth the densities are and the colours used. You can also add an
 * annotation to the chart indicated by a vertical line with an accompanying text annotation.
 *
...
class DensityChartButtonResponsePlugin implements JsPsychPlugin<Info> {
  ...
  private createDescription(html: string, display_element: HTMLElement): void {
    ...
    descriptionEl.id = "jspsych-densitychart-button-response-description";
  ...
  private createChartCanvas(display_element: HTMLElement): HTMLCanvasElement {
    ...
    chartContainer.id = "jspsych-densitychart-button-response-chart-container";
  ...
  private createButtonGroup(
    ...
    buttonGroupElement.id = "jspsych-densitychart-button-response-btngroup";
  ...
  private computeDensity(
    ...
    const grid: number[] = Array.from({ length: nPoints }, (_, i) => minValue + i * step);

    const gaussian = (u: number): number => (1 / Math.sqrt(2 * Math.PI)) * Math.exp(-0.5 * u * u);

    const density = grid.map((x) => {
      const sum = rawData.reduce((acc, v) => acc + gaussian((x - v) / bandwidth), 0);
  ...
  private renderChart(
    ...
    if (trial.highlight_score != null) {
      highlightIndex = Math.round((trial.highlight_score - grid[0]) / (grid[1] - grid[0]));
    ...
    if (!ctx) {
      throw new Error(
        "plugin-densitychart-button-response: Your browser does not support canvas rendering."
      );
      return;
    }
  ...
  trial(display_element: HTMLElement, trial: TrialType<Info>): void {
    ...
    trial.description_text && this.createDescription(trial.description_text, display_element);
    ...
    const after_response = (choice: number) => {
      this.handleResponse(choice, start_time, response, buttonGroupElement, trial);
    };
